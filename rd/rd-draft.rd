=begin
= RD working draft -- 20000423

== What is RD?

RD is Ruby's POD, embedable documentation format in script file. 
...
We will use a term "Element" for features of RD which add information
...
Paragraph, headline or list is a Block. RD use indent and special
characters to describe Blocks. You can also write complicated
structure to nest Blocks. And this style of Block looks naturally like
plain text format.  See ((<Block>)) for more details.

Emphasis, code or reference is an Inline. Parentheses and special
characters, like (('((? ... ?))')), are used for Inline. Almost all
Inline can nest each other. See ((<Inline>)) for more details.

=== Block
==== Basic Syntax

Block has line oriented syntax. i.e. each character of same line belong to a
same Block. And indent of line represent nesting level and type of Block.
First character of line represent type of Block.
...
:Head Char
  Head Char is first character of line, but except white spaces.

:STRINGLINE
  STRINGLINE is line which consists of strings.
  STRINGLINE doesn't have "(({*}))", "(({(((|num|)))}))", "(({:}))", "(({=}))"
  nor "(({+}))" as Head Char. But if line is indented, "(({=}))" and "(({+}))"
  can be Head Char.

:WHITELINE
  WHITELINE is a line consists of white spaces, "(({\s}))", "(({\t}))" and
  "(({\n}))".

:Comment
  A line which is matched with (({/^#/})) is regarded as comment.
...
by first Block of ListItem. For example,
  ...
  Example: "|" means Baseline
    ...
    |<- So this Baseline is at left most.
      *|it is in List. (1)
       |<- this Baseline is defined by (1) line.
      *    |Differnt Item of List can have different Baseline inside.
           |<- So this Baseline differ from one of first item's.
|Regard this line as one of TextBlock's in top level.
|<- So this Baseline is at left most.
  *|it is in List. (1)
   |<- this Baseline is defined by (1) line.
...
If one Block is nested in other Block, Baseline of inside Blocks is deeper
than Baseline of outside Blocks.

Relative position bitween Baseline and indent affect on type of Block.
...
Headline Mark represents level of Headline. See following figure. fisrt
Mark is biggest one, and last Mark os smallest one.
  Fig: Headline Marks
  =
  ==
  ===
...
:(('((:Term:))')) => ((:Term:))
    Term of Index.
:(('((<Identity or URL>))')) => ((<Indentity or URL>))
...
When you want to refer a resource pointed with URL, write such like:
  ((<URL:www.ruby-lang.org/en/raa.html>))
...
Reference, and when you want to write "|" inside it, you should write
such like:
...
When display text part is omitted, you can use ((<Inline>)) for Label.
   ((<((*Label*))>))
((<((*Label*))>))

== Topic of Discussion
We are discussing now about some topic of RD. Some of then is like followings.
  * Label conflict
  * Header

Please join our discussion in ruby-list and ruby-talk ML. Get more
information at ((<URL:http://www.ruby-lang.org/en/ml.html>)) in English,
and ((<URL:http://www.ruby-lang.org/ja/ml.html>)) in Japanese.
