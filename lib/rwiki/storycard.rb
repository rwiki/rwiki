...
EOS
    ...
    class ItemSection < Section
      ...
      ERBLoader.new('default_src(name)', 'story-item.erd').load(self)
    end
    ...
    class StoryCardIndexFormat < PageFormat
      @rhtml = {}
      @rhtml[:view] = ERBLoader.new('view(pg)', 'story-index.rhtml')
      @rhtml[:control] = ERBLoader.new('control(pg)', 'story-control.rhtml')
      reload_rhtml
    end

    class StoryCardTableFormat < StoryCardIndexFormat
      @rhtml = { :view => ERBLoader.new('view(pg)', 'story-table.rhtml')}
      reload_rhtml
    ...
    class StoryCardTestFormat < StoryCardIndexFormat
      @rhtml = { :view => ERBLoader.new('view(pg)', 'story-test.rhtml')}
      reload_rhtml
    ...
    class StoryCardPlanFormat < StoryCardIndexFormat
      @rhtml = { :view => ERBLoader.new('view(pg)', 'story-plan.rhtml')}
      reload_rhtml
    ...
    def install(name, base_name, pattern)
      ...
      Book.section_list.push(item_section)
      index_section = IndexSection.new(nil, name, base_name, item_section)
      Book.section_list.push(index_section)
    end
