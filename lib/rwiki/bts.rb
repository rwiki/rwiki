...
EOS
    ...
    class ItemSection < Section
      ...
      ERBLoader.new('default_src(name)', 'bts-item.erd').load(self)
    end

    class TableFormat < PageFormat
      @rhtml = { :view => ERBLoader.new('view(pg)', 'bts-table.rhtml')}
      reload_rhtml
    ...
    class IndexFormat < PageFormat
      @rhtml = {}
      @rhtml[:view] = ERBLoader.new('view(pg)', 'bts-index.rhtml')
      reload_rhtml
    end
